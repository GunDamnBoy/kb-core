...
流程：掃 outbox → verify（閘門）→ 不可改寫守衛 → 原子寫入 data/ →
      更新 index → pull --rebase → commit → push → 寫回執
...
def publish_one(draft_path: Path, repo: Path, outbox: Path) -> int:
    ...
    if target.exists():
        if target.read_text() == body:
            write_receipt(outbox, name, Exit.OK, "already-published",
                          f"{target.name} 內容相同，跳過")
            return Exit.OK
    ...
    write_receipt(outbox, name, Exit.OK, "pushed", f"{target.name} 已發布", sha)
    draft_path.unlink()
    return Exit.OK
...
def main(argv) -> int:
    if len(argv) != 3:
        print(__doc__)
        return Exit.BAD_INPUT
    outbox, repo = Path(argv[1]), Path(argv[2])
