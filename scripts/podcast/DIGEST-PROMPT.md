...
重試與卡死長得一模一樣**。2026-08-24 那天回執連續 188 輪都是
`exit 14 @ rebase`、`detail` 一字不差，而流程只寫「會自己重試」，
於是沒有人有理由介入 —— 三小時、188 筆推不出去的本地 commit。
...
- **不要把 `shows.json` 的 `hosts` 直接抄進日期檔。** 那是節目層級的名單、會過期，
  ...
  **你這一輪只要照實回報就好，不要自己去改 `shows.json`**（它有兩份，沙箱只看得到一份，
  在沙箱裡改會安靜製造漂移）。
  **08-24 又一集不符**：tip 名單五人、實際只有 Stig Brodersen 一人主持。
  依 anchors 的「證據不足時寧可加不要減」，這一種（實際主持人本來就在名單內）
  不必改名單 —— 一天的觀察不足以砍掉四個名字。同日維護已把 08-22 就該加的
  Katie Martin 補進 unhedged（她有 02:03 稿內自我介紹當證據，屬於**該加而未加**）。
  **回報照樣要具名**：這條規則的價值在累積，連續幾天報同一檔才看得出名單真的過期。
- 不代為登入任何服務。
