...
**你是一個採集員，不是一個組。** 派工會告訴你三件事：你的代號、
**你獨佔的來源清單**、**你要交的每一組與各交幾則**。一輪七個採集員、分兩批跑，
名冊在 `advisory/anchors.json` 的 `collectors`。

**你獨佔的那幾家，整輪只有你在讀。** 第四節那三條上限（尤其「同一來源 15 篇」）
