...
| **CME 金屬結算表**（`metals/precious/gold.settlements`） | 只有交易日有；**週五結算會在台北週六 12:55 貼出**，那是週末窗口唯一穩定的「新發布」。**輪詢條件用 `document.querySelectorAll('table tr').length > 3`，08-30 實測 500ms 就滿足**，不必等秒數 | 09-06 實測 TRADE DATE `Friday, 04 Sep 2026`、頁註 `04 Sep 2026 11:55 CT`，14 列 |
