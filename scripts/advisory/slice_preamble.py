...
SECTION_OWNER = {
    ...
    "## 九、回報格式":         ALL,
    # 2026-08-31 新增。「時刻要標量到的還是估的」是回報紀律，不綁任何來源，
    # 所以是 ALL —— 判準與 `### 中文來源`／`### 發稿日曆` 那兩列不同：
    # 那兩列篩的是「表上出現了誰的來源」，這一列根本沒有表。
    "### ⚠️ 你寫的每一個時刻": ALL,
}
...
ROW_OVERRIDE = {"ECB（官方站）": "央行官方", "美國財政部": "央行官方",
                "Fed 官方演說": "央行官方",
                # 2026-09-09 新增 BOJ 官方英文站那一列。roster 裡 A 的來源字串是
                # 「央行官方（Fed／BOJ／ECB／美國財政部）」，四家共用一個條目，
                # 所以四列都要在這裡對映回同一個關鍵字。
                "BOJ 官方英文站": "央行官方"}
