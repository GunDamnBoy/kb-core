...
CORE = [
    ...
    # 10 年期損益兩平通膨率（2026-09-17 加）。
    # **它是「名目殖利率」唯一的拆解工具**：有它才畫得出「這一段利率上行是實質還是通膨」，
    # 而那是 2026-09 這一波最常被問的一題（上游卡 11 就在問它）。
    # 加進來的直接原因是它把 2026-09-17 那一輪的當日主圖擋掉了：
    # 快取裡有 `T10YIE`，但它從 2026-08-12 起就沒有被刷新過 ——
    # 不在 CORE、也掉出了近 14 天的窗口，於是**沒有任何一行輸出提到它**，
    # 直到選完題、寫到一半才發現末日落後 24 個交易日。
    # 同一天新增的 `prep_chart._uncovered()` 讓這種序列在選題前就現形；
    # 這一行則是讓它根本不會變舊。
    # **驗收點是 2026-09-18 11:00 的第一次預抓**：落進 `failed` 就把它從這裡拿掉，
    # 不要留著讓它每天貢獻一條假的失敗。它與 `DGS10` 同源（FRED，日頻），風險低。
    "T10YIE",
    # 2026-09-17 晚間，清「快取有、涵蓋沒有」那 27 條時加的兩條。
    # **兩條都有具體的需求證據，不是「可能會用到」** —— 同一次盤點把
    # `HYG`／`LQD`／`RSP` 也考慮過，而近 28 期的 `series_spec` 對它們的命中是 0，
    # 所以它們去了刪除那一堆。**沒有需求訊號就加進 CORE，是每天替沒人看的東西打一次外部站台。**
    #
    # `DCOILWTICO`：西德州原油（FRED，日頻）。需求有三個：週四軌道是原物料與能源而我們只有布蘭特；
    # 上游每天有 WTI 官方結算的卡（2026-09-17 是卡 75）而我們畫不出來；
    # 以及 `SOURCES.md` 那個未結的布蘭特口徑落差 —— **有第二條油的序列才有得對照**。
    # 它同時讓 `CL=F` 變成一條真正「已被取代」的別名（見 fetch.BLOCKED），與 `BZ=F` 成對。
    "DCOILWTICO",
    # `8069.TWO`：**它是 `anchors.dead_series.^TWOII` 的對照組**，那一格逐字寫著
    # 「同一天 8069.TWO（櫃買個股，走櫃買官方端點）末日正常，所以不是櫃買資料拿不到，
    # 是櫃買的『指數』在這條路上死了」。而它從 2026-09-02 之後就沒有再被抓 ——
    # **一個沒有在更新的對照組，證明不了任何事**，那句登錄會安靜地變成舊聞。
    # 它也是台股路線，快取深度補過（見 history_limits.tw_route_months），刪掉要重跑 backfill。
    "8069.TWO",
