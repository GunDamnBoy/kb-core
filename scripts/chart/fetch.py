...
BLOCKED = {
    ...
    # 2026-09-17 加。`prep_chart._uncovered()` 上線後盤了一次「快取有、涵蓋沒有」的 27 條，
    # 逐條問「有沒有等價序列，而且我們真的在抓」—— **只有這一條答得出來**
    # （`^DJI`／`^NDX`／`^RUT`／`^FVX`／`CL=F`／`RB=F`／`TWD=X` 都沒有在抓的等價序列，
    # 所以它們不是「被取代」，是「沒人在用」，那是去留問題不是登錄問題）。
    # **這一格現在有第二個讀者**：`_uncovered` 拿它把「已有替代來源」與
    # 「沒有人在管」分開印 —— 登錄一條，清單就短一行，**讓登錄有立即可見的回報**。
    "DX-Y.NYB": "DTWEXBGS",
    # 2026-09-17 晚間加，與上面的 `BZ=F → DCOILBRENTEU` 成對。
    # 在此之前 `CL=F` 是孤兒：它的雙胞胎已經登錄了，它自己沒有，
    # 而**同一類的東西登錄了一半，比兩個都沒登錄更難發現** —— 表上看起來已經處理過了。
    # 這一條要成立的前提是我們真的在抓 `DCOILWTICO`，所以它與 `prefetch.CORE`
    # 那一行是同一次改動的兩半，**拆開做會得到一條指向空氣的登錄**。
    "CL=F": "DCOILWTICO",
}
