...
**`14` 而 `stage` 是 `git` 是例外**：那是 `git` 這支指令本身失敗，`detail` 裡有它的
stderr。人修好之前重試不會好，修好之後下一輪就會自己完成 —— 草稿不必改。
**沒有回執也不一定等於 publish 沒跑**：2026-09-15 至 09-16 它是跑到最後一步才丟
未攔截的例外（已於 09-16 攔成回執）。
