...
**第一頁抽不到的那種會擋整批 —— 但 2026-08-31 起它會自己換軌。**
`extract.py` 遇到「主抽取器把第一頁抽成空的」（塌縮空白後不足
`anchors.page_one_is_the_thesis.min_visible_chars`）時，**只有那一份**改用
`engine_order` 的下一支重抽，並把 `engine` 與 `engine_fallback` 記進抽取結果。
它會印一份具名清單，**你要把那份清單抄進回報**。

代價是那一批變成混軌，`research.one_engine` 出 WARN —— **那是預期的，不要去消掉它。**
兩軌對旋轉文字、欄位與空白的處理不同，所以換過軌那一份的**剃除佔比不要拿去跟別份比**；
份內的原句與 grounding 不受影響（每一份只跟自己那份的抽取文字逐字比對）。

> ~~**第一頁是圖檔的那種會擋整批（2026-08-30 遇到）。** 高盛《Global Semis: CHIPS IV》
> 第一頁只有浮水印，剃完 0 字元……當輪的處置：把抽取結果 `mv` 到 `_pending/`，
> 交給使用者裁決。~~
>
> **2026-08-31 實測推翻：第一頁不是圖檔。** `pdftotext` 對它只吐出 113 個字元
> （全部是浮水印），而 `pdfplumber` 對**同一頁**吐出 3,390 個字元，報告日期就印在
> 第一行（`EQUITY RESEARCH | August 24, 2026 | 12:06 AM HKT`）。
> 上面那段是**看到浮水印以外沒有東西就下的推論，沒有換一支抽取器試過**，
> 而它被寫進這份正本當成了事實。
>
> 留著劃掉的原文，是因為它示範的正是這份文件別處也記著的形狀：
> **「抽不到」與「不存在」在輸出上長得一模一樣**，而
> **「預期中的失敗」寫進正本之後，它會一直被當成事實，直到有人真的去跑一次。**
...
**回執 exit 0 也不代表讀者看得到。** 站台是 GitHub Pages，而 `deploy.yml`
在上傳之前有兩道內容守衛（`verify_site_data`、`verify_site_index`）——
**其中一道紅了，Pages 就不部署，而 publish 那一側三個訊號全是綠的**：
...
收了幾份、每份的券商與日期、**抽取器是哪一支、有沒有哪一份換過軌**
（兩軌對同一份檔可能給不同答案；換軌的清單 `extract.py` 會印出來）、
