...
**管線指令跑在 `device bash`，而那是一個 Linux 沙箱、不是 Mac 的 shell。**
授權的資料夾掛在 `$HOME/mnt/` 底下，**`/Users/macmini` 與 `~/.venvs/kb/bin/python`
在那裡都不存在** —— 用沙箱自己的 `python3`。

**路徑一律明講，不要靠預設值。** `BROKER_RESEARCH_ROOT="$HOME/mnt/broker-research"`，
並且對每一支管線程式顯式給 `--outbox` 與 `--repo` —— 那兩個旗標的 argparse 預設是
`~/...`，在沙箱裡**會安靜地寫到錯的地方**。`_paths.py` 的檔頭本來就寫著
「Cowork 階段照 `RUN-PROMPT` 明寫掛載路徑」，而在 2026-08-24 之前這份並沒有寫。
...
**已經畫過的圖不重畫（2026-08-31 加）。** 它會印一行
`圖：重畫 N　沿用 M　認養 K`——**那一行就是這條規則有沒有生效的證據**，
少了它代表你跑到的是舊版。判準是規格本身的 sha256（存成
`charts/<base>.spec.sha`，不進 repo），規格沒變就連渲染都不跑。

理由是 2026-08-30 那一輪：每日五圖那邊改了 `chartkit.py` 與 `chart/anchors.json`，
`timeseries` 的輸出換了位元組數（129,550 → 132,268）。**規格一字沒動、`bytes` 卻變了**，
於是不可改寫守衛判「已發布的內容被改了」，W35 exit 11、W34 連帶 exit 15。
渲染本身是決定性的（同一份規格連畫兩次 md5 相同），所以那不是雜訊，
是上游改版倒灌進已發布的期別。**那次只中一張；下次改到 `grouped_bar`，
W34 那 25 份會一起被指名。**

要強制重畫**刪掉那張的 `.png`**，不要刪 `.spec.sha`——後者會走「認養」，
把現有那張當成最新的收下。
...
>
> **2026-08-24 實測推翻：沒有被擋。** 那一輪發布前拿 `kbcore.system.append_only`
> 對已發布的 `data/2026-08-23.json` 與新 payload 實跑一次，回 `None`；
> 23 份逐份 JSON 比對零差異，那條 grounding 也一樣。
> **8-23 那次發布已經把換抽取器之後的版本一起送出去了。**
> 留著這幾行是因為它示範了一件事：**「預期中的失敗」寫進正本之後，
> 它會一直被當成事實，直到有人真的去跑一次。**
