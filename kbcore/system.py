...
class System:
    ...
    **沒有預設值是刻意的。** 給一個 `["data"]` 的預設，等於讓下一套系統
    ...
    2026-08-22 加的。哨兵原本把 `MAX_DATE_AGE_H = 36` 寫成模組常數 ——
    那是日頻的答案，而它對四套裡的每一套都適用，直到第五套是週頻的為止。
    週頻套上 36 小時的門檻，**每一週都會紅六天**，
    然後那個哨兵會在兩週內被所有人略過。

    > **一個對所有系統都一樣的門檻，只是還沒遇到不一樣的系統。**

    必填，沒有預設 —— 理由跟這個 dataclass 其他欄位一樣：
    給預設等於讓下一套系統安靜地繼承錯的節奏。

    日頻填 24、週頻填 168。哨兵的容忍度是它的 1.5 倍
