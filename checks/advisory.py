...
register(Check(
    ...
    covers="**派工設定本身**合不合理：每組每模式都有負責人、指派則數加總等於配額、"
           "每個採集員恰好在一個批次裡。讀的是 anchors，不是當日產出",
    blind_to=[
        "**這是設定檢查，不是產出檢查——它一個當日欄位都沒讀。**"
        "2026-08-21 用讀取追蹤量到：其餘 16 條各讀 3–13 個產出欄位，這條讀 0 個。"
        "所以它會永遠 PASS 直到有人改 anchors，卻佔著「17 條檢查」裡的一格，"
        "讓那份報告讀起來像當日產出被驗了 17 次",
        "**「設定說七個採集員」與「今天真的派了七個」是兩個問題**，這條只答得出前者。"
        "後者目前沒有任何機器在問——日檔裡沒有派工紀錄，"
        "唯一的間接訊號是窗口收盤時間（跑得慢就晚）",
        "指派合理但那個人今天沒交東西（那是 group_floors 的事，而它只看總數不看誰交的）",
        "批次的順序與併行度（只驗每個人恰好在一個批次裡）",
    ],
    run=_dispatch,
    fixture={"anchors": {
