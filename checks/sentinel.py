...
def _no_future_date(p):
    ...
    #
    # **2026-08-31 訂正：對日頻是這樣，對週頻不是。**
    # 週頻那一套的日檔以**該週的週日**命名（`2026-08-30.json` ＝ W35），
    # 而報告在週間就會到、週間就會組檔 —— 週一組出來的那一期，
    # 檔名上的日期必然還在未來。那不是「日期被寫出來」，是**期別的標籤**。
    #
    # 所以容許量＝`cadence_hours - 24`：
    #   日頻 24-24 = 0 小時 → **行為與 2026-08-31 之前完全相同**（fixture 照樣觸發）
    #   週頻 168-24 = 144 小時 → 週一組出週日那一期，剛好在界內
    # 超前一整個週期以上仍然是紅的 —— 那才是「日期被寫出來」。
    #
    # 這條在 2026-08-25 到 08-31 之間讓 broker-research 的哨兵每次都 exit 10，
    # 而**哨兵一直卡在這條，就沒有機會說出「站台停在舊的一期」**——
    # 那八天站台真的停著，沒有任何一個警報講到那件事。
    lead = max(0, _cadence(p) - 24)
    if age < -lead:
        return fail(f"最新一期是 {days[0]['date']}，那一天還沒到（還有 {-age:.0f} 小時，"
                    f"容許超前 {lead:.0f}）—— 日期是被寫出來的，不是被觀測到的")
    return ok()


register(Check(
    id="sentinel.no_future_date",
    covers="days[0].date 不超前 cadence_hours − 24 小時（日頻 0、週頻 144）",
    blind_to=[
        "日期沒超前，但它是硬寫進去的當天日期而非真的抓到了資料",
        "**週頻容許 144 小時之內的超前**，所以週頻那一套把日期寫成本週週日"
        "與真的在週日產出，這條分不出來",
    ],
    run=_no_future_date,
    fixture={"now": "2026-08-19T01:00:00+00:00",
