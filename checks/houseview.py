...
# 不是某一組座標數字。
#
# ## 2026-08-23 下午：版面對齊那一版落地了，而它證明了上面那句話
#
# `build_hv3.js` 改成兩組設定檔（`house` 10×5.625 兩欄、`v3` 13.333×7.5 三欄），
# 而 `house` 的格線與**凍結的 `build_hv.js` 到小數點後兩位完全一致**
# （ML 0.27、COL_W 4.60、COL_X 0.27／5.13）—— 也就是說 v3 那組三欄常數
# 從頭到尾是偏離值，把它鎖起來會鎖住錯的東西。
#
# 那一版落地時，這裡唯一存活的座標型檢查 `rule_bodytop` 當場 FAIL
# （它比對 `let bodyTop = 1.62`，而程式改成了 `L.BODY_T`）。行為完全沒變，
# 是檢查自己在量錯的維度。已改成鎖關係，見下表那一列的註解。
#
# **還沒補回來的是「畫布與模板一致」那一條。** 它要驗的是設定檔裡的
# `house.W／H` 與交付模板的畫布相同，而模板不在 payload 裡 —— 補它要先讓
# `houseview_verify.py` 把模板畫布讀進 payload，那是另一件事。
...
for _id, _pat, _name in [
    ...
    # **這一條原本寫的是 `let bodyTop = 1.62`。** 它宣稱保護「動態內文起點」，
    # 實際量的是一個常數的字面值 —— 2026-08-23 畫布改成設定檔（`L.BODY_T`）之後
    # 它就 FAIL 了，而那個改動正是往對的方向走。**守衛量到的維度比它宣稱保護的
    # 東西低一層**，同一類這個月第六次。改成鎖那條關係本身：導言把內文起點往下推。
    ("rule_bodytop",   r"bodyTop \+= ",            "導言列（lede）把內文起點往下推（動態，不是常數）"),
