...
### 追加（同日稍晚）：布蘭特去問了 FRED，`weekly_release_series` 有一句是錯的

上面第四項寫「該問的是 `DCOILBRENTEU` 在 FRED 上的定義」。**當天稍晚就去問了**，
一次 `web_fetch` 序列頁，砍掉兩個可能性、也順手推翻了 anchors 裡的一句話。

**一、我們沒有抓錯，也不是單位問題。** FRED 頁上末五筆
（09-15 `130.80`、09-14 `121.25`、09-11 `118.06`、09-10 `120.98`、09-09 `109.51`）
**與我們的快取逐筆相同**；單位是 `Dollars per Barrel`、`NSA`、`Daily`，
來源 EIA、release「Spot Prices」（rid=212）。
所以「現貨 vs 期貨」這個解釋只剩一半成立 —— 單位與品級都相同時，
**兩個月的價差 22% 不是口徑能解釋的量級**。
下一步因此轉向上游那兩張卡的出處，不再查我們這一側。`SOURCES.md` 已改寫成這個結論。

**二、`anchors.freshness.weekly_release_series.DCOILBRENTEU` 原本寫著
「它與 H.10 那兩條不同的地方是**沒有公告的發布日**」—— 那是錯的。**
同一頁就印著 `Updated: Sep 16, 2026 12:17 PM CDT` 與 **`Next Release Date: Sep 23, 2026`**，
而那個 release 有自己的發布行事曆。

2026-09-13 由 `_prefetch_history.jsonl` 的 13 輪帳推出「7 天與 8 天兩次跳動」——
**結論是對的，方法是繞遠路的**。週頻成立，只是我們用觀測到的間隔，
去反推一件對方已經公告的事。

**它藏住的方式**：帳算出來的答案合理，於是沒有人再去問「對方有沒有直接說」。
這是 `release_cadence_ledger` 那條病歷的反面 —— 那一條講「帳早就在了，缺的只是讀者」，
**這一條講讀了帳就不會再去讀公告**。那一格已改寫，並加了一句
「下一個人要改這一格之前先去序列頁看一眼，那是第一手的，帳是第二手的」。

**兄弟條目也問了一次。** 既然三條週頻登錄裡有一條寫錯，另外兩條就不該只靠信任。`DTWEXBGS` 的序列頁：`Updated: Sep 14, 2026 3:16 PM CDT`（週一）、`Next Release Date: Sep 21, 2026`（週一）、末筆 2026-09-11（週五）、release H.10（rid=17）——**「每週一發布上一週的每日觀測」是對的**，那一格已加註「實地核對過」。

**順帶量掉一個沒有成立的疑慮**：本輪也懷疑過 `chart.series_freshness` 的 WARN
是不是又變成「天天都在響」（2026-08-30 換交易日尺治過一次的那個形狀）。
拿近 14 期封存實測 DGS10 的落後交易日數：**WARN 2 期、FAIL 0 期、乾淨 12 期**。
**沒有失效，不改。** 今天那一筆 WARN 是 FRED 09-16 沒有觀測值造成的真實落後。
記在這裡是因為**「查過而且沒問題」與「沒查」在輸出上長得一模一樣**。

### 追加（同日深夜）：`_uncovered` 分兩段，而「要開新登錄表」是我自己沒量就下的判斷

上面第二項上線後盤了一次那 27 條，本來的說法是「約 12 條是永遠不會再刷新的，
而 `dead_series` 管不到這一類，所以要開一格新的登錄」。**兩句都不準。**

**一、家已經存在，不用開新的。** `scripts/chart/fetch.py` 的 `BLOCKED`
（12 條，`別名 → 取代它的序列`）就是在回答「這條已經被誰取代」，
只是在此之前它**只有取數層一個讀者**。

**二、真正符合「已被取代」的只有 1 條，不是 12 條。**
逐條問「有沒有等價序列，而且我們真的在抓」：

| 別名 | 可能的等價 | 我們在抓嗎 |
|---|---|---|
| `DX-Y.NYB` | `DTWEXBGS` | **是** |
| `^DJI`／`^NDX`／`^RUT`／`^FVX`／`CL=F`／`RB=F`／`TWD=X` | DJIA／無／無／DGS5／DCOILWTICO／—／DEXTAUS | 都沒有 |
| `GDP` ＋ 三條 BEA 季頻 | — | 沒有 |

所以那 11 條**不是登錄問題，是「這份快取還要不要留」的問題** ——
它們沒有被取代，只是沒有人在用。
**把「沒人用」講成「已被取代」，是沒量就先分類** —— 與同一天早上那次誤判同形。

### 改了什麼

- `fetch.BLOCKED` 加 `"DX-Y.NYB": "DTWEXBGS"`（唯一答得出來的那條）。
- `_uncovered()` 讀 `BLOCKED`，分兩段印：已登錄的收進一行摘要，沒登錄的逐條列，
  **標題仍然數全部**（少印不等於變少）。
- `_blocked_map()` 包成「讀不到就回空字典」：`fetch` import 時解析資料 repo，
  取不到會 `sys.exit`，而這支是唯讀盤點表。**接的是 `BaseException`** ——
  `SystemExit` 不是 `Exception` 的子類，只接 `Exception` 會讓它跟著死。
  實測 `CHART_REPO=/nonexistent` 時 `fetch` 確實 exit 1，而 `_blocked_map()` 回空字典、不掛。

**為什麼要讓 `BLOCKED` 有第二個讀者**：登錄一條，清單就短一行 ——
**一個沒有回報的登錄動作不會有人去做**。

**量測（實跑）**：27 條 → 摘要行 2 條（`^STOXX50E→FEZ`、`DX-Y.NYB→DTWEXBGS`）
＋ 逐條列 25 條。**立即的縮減很小，這一點要說清楚**：真正的縮減在那 11 條的去留決定，
而那是使用者的決定，不是這一輪能代做的。`T10YIE` 還在清單上，
因為狀態檔是今天 11:00 的（早於它進 `CORE`），明天預抓之後會自己掉出去。

**回歸**：`selftest_offline()` 加四條 —— 標題仍數全部、已登錄的收進摘要行、
已登錄的不逐條列、沒登錄的仍逐條列。**只驗「有沒有變短」不夠：把整段刪掉也會變短。**

### 順帶補進帳的一列

`usage.csv` 的 `2026-09-16 chart` 缺列已在 Mac 上用
`usage_scan.py --date 2026-09-16` 補回（**25.7 分鐘／3,153k／84 輪，`bounded=window`**，
逐字稿 `d3cfc3a5-…`）。**它同時是那個 glob 修正的端到端驗收**：
09-16 那一場正是短格式 session 目錄，修之前掃描對它一份都掃不到。
乾跑印的上界 `2026-09-16T11:56:53+08:00` 與該日日檔的 `window.to` 逐秒相符 ——
**挑錯逐字稿不會剛好對上另一個檔案裡的時刻。**

### 追加（同日深夜之二）：清掉 23 條沒人在管的快取，而量測推翻了我自己的分堆

上一段把 25 條分成三堆並給了建議。**照建議執行之前先量了一次，第一堆當場垮掉。**

**量法**：近 28 期日檔（2026-08-21 至 09-17）逐份找 `"id": "<代號>"`，
也就是真的被畫成圖的那種引用。**先跑控制組確認量尺有效** ——
`SOXQ` 17 期、`^TWII` 16 期、`GLD` 13 期、`SP500` 12 期、`MU` 7 期，量尺沒問題。

**結果：24 個候選全部是 0 期。** 包括我上一段說「可能還想用」的
`HYG`／`LQD`／`RSP`。**沒有需求訊號就加進 `CORE`，是每天替沒人看的東西打一次外部站台** ——
這與 2026-09-13 那四條的加法剛好相反，那一次的理由是「上游天天有卡而我們畫不出圖」，
是**有**需求訊號的。

**我上一段的分堆是憑印象分的，而量測把第一堆整個推翻。** 這是同一天第三次同形的錯：
早上把「我找不到」當成「它不存在」，傍晚把「沒人用」當成「已被取代」，
現在是把「聽起來像會用到」當成「需求」。**共同點是都沒有先量。**

### 最後做了什麼

**進 `CORE` 兩條，各有具體證據**（不是「可能會用到」）：

- **`DCOILWTICO`**（西德州原油）：週四軌道是原物料與能源而我們只有布蘭特；
  上游每天有 WTI 官方結算的卡（09-17 是卡 75）畫不出來；
  以及那個未結的布蘭特口徑落差 —— **有第二條油的序列才有得對照**。
- **`8069.TWO`**：它是 `anchors.dead_series.^TWOII` 逐字寫著的**對照組**
  （「同一天 8069.TWO 末日正常，所以不是櫃買資料拿不到」），
  而它從 2026-09-02 之後就沒再被抓 —— **一個沒有在更新的對照組證明不了任何事**，
  那句登錄會安靜地變成舊聞。它也是台股路線、深度補過，刪掉要重跑 backfill。

**`fetch.BLOCKED` 加 `"CL=F": "DCOILWTICO"`**，與既有的 `BZ=F → DCOILBRENTEU` 成對。
在此之前 `CL=F` 是孤兒：**同一類的東西登錄了一半，比兩個都沒登錄更難發現** ——
表上看起來已經處理過了。這一條與上面 `CORE` 那一行是同一次改動的兩半，
**拆開做會得到一條指向空氣的登錄**。

**23 份快取搬進 `chart-of-the-day/_to_delete/series-stale-20260917/`，不是 `rm`** ——
照這個 repo 處理舊 checkout 的同一套做法（`~/_to_delete/advisory-knowledge-hub-stale-20260818`）。
保留 `DX-Y.NYB` 與 `^STOXX50E`：它們已登錄替代來源，現在只佔摘要行的兩個名字，
**留著正好示範「登錄一條、清單就短一行」**。

### 量測與驗收

**那一段從 27 行變成 4 行**（摘要行 2 條 ＋ 逐條 2 條），
而剩下的 `8069.TWO` 與 `T10YIE` 兩條都已經進了 `CORE` ——
**明天 11:00 預抓之後逐條那一段會是空的**，這同時是那兩條的驗收點：
明天 prep 若它們還在，代表預抓失敗了，照 2026-09-13 的規矩把它們從 `CORE` 拿掉。

**確認不會自己長回來**：`recent_ids()` 只看近 14 天用過的序列（現為 33 條），
搬走的 23 條**沒有任何一條**落在 `CORE ∪ recent`，所以明天不會被重新抓。

**確認不影響封存**：日檔把 `series` 存在自己裡面，快取只服務新圖。
41 份日檔全部 `json.load` 正常，抽驗 09-15 五張圖的資料點數仍是 751／4／3／1／4。

**下一次 publish 會把那 23 個刪除提交上去**（`staged_paths` 回傳 `["data", ...]`，
而 `data/series/` 有進版控）。`_to_delete/` 不在 `staged_paths` 裡，留在本機。
