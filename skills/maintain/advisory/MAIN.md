...
- **`SKILL.md` 引用 `preamble` 某一節「現行結論」的地方，那個結論是不是還有效。**
  這是 2026-09-08 抓到的一種漂移，**而它不是「兩份副本不一致」，是「正本改了、讀者沒跟上」** ——
  所以前面那幾條比對副本的檢查全部看不到它。實例：`preamble` 第六之二節在 09-06
  把注入文字的判準從「數次數」改成「出現在哪裡」、並明寫「派工端不必再指定刻意開一篇 STAT+」，
  而 `SKILL.md` 步驟 3 **連續兩輪還在要求那個動作**，派工端每天都在派一個已經被撤銷的工作。
  **查法**：`grep -n 'preamble\` 第' skills/advisory/SKILL.md` 列出所有引用點
  （2026-09-08 實測 14 處、涉及 5 個章節：第三、第四、第六、第六之二、第七之二），
  逐一回去讀那一節現在的結論，比對 `SKILL.md` 轉述的版本。
  **`preamble.md` 那 5 節底下各有一行 `**↩ 被引用：**` 的反向指標**（09-08 加），
  是給改動端看的另一半護欄 —— **它不會進採集員的切片**（`slice_preamble.py` 的
  `strip_maintainer_only()` 會剝掉），所以加它不花採集端的 context。
  **兩邊都要在**：反向指標守「改的那一刻」，這一條守「事後覆核」。
  **⚠️ 目前沒有任何程式在看這件事**，兩次都是人讀出來的（一次是採集員同時拿著切片與任務卡、
  一次是維護者在步驟 1 讀保底檔時）。**它的偵測延遲約兩輪。**
