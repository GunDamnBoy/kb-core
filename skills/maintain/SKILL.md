...
**桌面排程器的自述欄位都不能當證據**：一輪確實跑完、發布成功時，`list_scheduled_tasks` 的
`lastRunAt` 可能沒有動；`nextRunAt` 在**派工當下**就往前推進，所以它跳過去既不代表跑完、
也不代表被丟掉。

**唯一可信的是產出檔**：`~/outbox/<目錄>/<日期>.receipt.json`（advisory 在 outbox 根目錄）。
漏跑可能是特定幾支、而不是一段時間窗口，所以逐支用產出檔判。

### 哨兵：`tools/schedule_gaps.py`

`com.kenny.kbschedgaps` 每天 **13:10** 跑，只讀產出檔，寫 `~/.kbusage/schedule-gaps.md`。
**它跑在 launchd 上，不是桌面排程器上** —— launchd 全綠時桌面排程器仍可能漏跑，
**哨兵掛在被監視的對象上就會跟著一起安靜**。
