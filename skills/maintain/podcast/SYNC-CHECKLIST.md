...
本節有兩組：`DIGEST-PROMPT` ↔ 排程 `SKILL.md`，以及
**`kb-core/skills/maintain/` 正本 ↔ 已安裝的技能副本**。
...
- `diff` 正本與副本，**只差 front matter 五行才算同步**。
  **不要用行數當判準**（副本檔尾可能沒有換行符，`wc -l` 會誤報成「看起來不同步」）。
...
**比對前先確認本檔沒有落後**——用來查漂移的東西自己漂了是最難發現的一種，因為沒有人拿它去查它自己。
以下任一條成立，就先補本檔再比對：

1. 本檔 mtime 比 `MAINTENANCE.md` 第 12 節 `### 逐場登記` 速查表最新一列的日期舊（速查表是索引，每一場都必須有一列）。
   讀那一場，確認它有沒有新增「需要比對的東西」——光比 mtime 只知道它舊了，不知道舊掉的是哪一塊。
2. 本場動了 `healthcheck.py` 的 `main()` 檢查串列、`checks/podcast.py` 的檢查清單、或任何 `systems/*.py`——
   這三處是新增比對關係的來源，當場就問「這新增了哪一組需要比對的東西」。登記簿收工才寫，
   而複驗跑在收工之前，所以第 1 條抓不到本場自己漏的。
3. 速查表最新一列比今天舊超過 7 天——先分清楚是真的沒有場次，還是有人只寫了第 12 節上方的
   `### YYYY-MM-DD` 區塊而沒進速查表；後者的話，補上那幾列，並把那幾場的比對關係補進本檔。
