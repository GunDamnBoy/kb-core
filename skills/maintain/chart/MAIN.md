...
- **`blind_to` 裡有沒有混進「還沒修」。** 2026-09-10 抓到的那一類，而它是這張清單上
  **唯一一條要往回讀既有文件、不是往前比對兩份實作的**。
  `chart.series_freshness` 的 `blind_to` 逐字寫著「日頻資料剛好落在 1 號會被誤判成月頻」——
  **那句話是對的，位置是錯的**：`blind_to` 的用途是「這條檢查驗不到什麼」，
  而這一筆是「這條檢查寫錯了」。兩者混在同一張清單上，**前者會替後者提供掩護** ——
  它被正確地發現、寫下來、歸檔，然後就以「已知限制」的身分活了下來，
  直到 2026-09-10 真的咬到（`DCOILBRENTEU` 落後 7 個交易日而兩側同時判它沒事）。
  查法是把每一條 `blind_to` 與 `known_limits` 問一次：
  **這是「驗不到」還是「還沒修」？** 前者才該留在那張清單上；
  後者要嘛修掉，要嘛帶著「為什麼現在不修」一起寫。
