...
**涵蓋範圍（2026-09-14 起）**：`reports` 逐份比對（改了或不見了都擋），
加上 `crosscut`／`watch`／`notes` 的「只准長大」——
字串要求舊的是新的前綴，串列要求舊的每一條還在原位。
那三欄在 2026-09-14 之前**完全不在守衛裡**，實測改了回 `None`。

**補登舊報告時的順序**：要改那三欄就**先改、先發布，再收緊**。
反過來會把自己鎖在外面 —— 本輪的 W35／W36 crosscut 就是在守衛收緊之前重寫的。
之後只能往後接。

**`append_only` 只有 research 一套在用**（advisory／chart／podcast／tracer 是 `frozen`，
convergence 是 `errata_only`），所以動它的波及範圍比「五套共用」那句話小。
**但它住在共用檔裡，所以改完仍要跑一次 `report.selftest()` 確認自檢契約沒被打壞。**
