...
- **`crosscut`／`watch`／`notes` 被改寫** → 擋（2026-09-14 起）。
  那三欄在此之前**完全不在守衛裡**，可以任意改寫而它一聲不出。
  現在跟報告同一條規矩：**舊的原封不動留著，新的接在後面**
  （例：`**（YYYY-MM-DD 補登之後）**` 起一段）。
  **補登一定會改變「這一週長什麼樣子」**，所以這條刻意不是 `frozen` —— 照著往後接就會過，
  不必繞；而一個每次都得繞過的守衛，遲早會被繞過它不該被繞過的那一次。

**exit 10（閘門）與 exit 11（守衛）是兩回事，回執的 `stage` 會說是哪一個。**
補登舊報告最容易撞的是 exit 10 的 `research.ledger_no_overdue`：
立場的 `due` 是報告日 + 3 個月，**一份幾個月前的報告一進帳本就已經逾期**，
而 `data/stances.json` 是全域檔 —— **一份舊報告會擋住所有期別的發布**。
2026-09-14 起 `filed`（入庫日）是第二條線，逾期要同時超過兩條。
真的遇到逾期時**不要代為批量改判「無法驗證」**：那條檢查的 `blind_to` 自己寫著，
那正是它最容易誘發、而它擋不住的行為。
